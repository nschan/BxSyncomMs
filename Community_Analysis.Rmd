...
```{r}
random_jac_site_aov <- vegan::betadisper(random_jaccard, paste(labels(random_jaccard) %>% str_extract("APO|BOA|DMSO"),
  labels(random_jaccard) %>% str_extract("24h|48h|72h|96h"), sep = "_")) %>% 
  vegan::scores() %>% 
  .$sites %>% 
  as.data.frame() %>% 
  ...
  rownames_to_column("Sample") %>% 
  pivot_longer(2:3, names_to = "PC", values_to = "Distance") %>% 
  separate(Sample, c("Drop1","Syncom","Treatment","Drop2","Drop3", "Rep" ,"Timepoint"), sep ="_") %>% 
  dplyr::select(-starts_with("Drop")) %>% 
  group_by(PC) %>%
  nest() %>% 
  dplyr::mutate(.data = .,
                aov_results = data %>% purrr::map(.x = ., .f = ~ aov(Distance ~ Treatment*Timepoint, data = .)))
```
```{r}
random_jac_site_aov$aov_results[[1]] %>% TukeyHSD()
```
...
```{r}
plot_ordination(phyl_random_rel_strain,
                cca_jaccard_random_rel_strain,
                color = "Treatment",
                axes = c(1,2)) +
  ggforce::geom_mark_ellipse(fill = "grey90") +
