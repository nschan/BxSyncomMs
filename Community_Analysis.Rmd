...
left_join(strain_AUC_deltas, strain_relabund_delta,  by = c("Strain", "Treatment")) %>% 
  filter(!is.na(Treatment),
         ...
         !is.na(mean_rel_abund),
         Treatment != "Control")  %>% 
  ggplot(aes(x = mean_rel_AUC, y = ratio_rel_abundance, color = Treatment)) +
  geom_vline(aes(xintercept = 1)) +
  geom_hline(aes(yintercept = 1)) +
  geom_point(colour = "black", size = 4.5, alpha = 0.7) +
  ...
  theme_bw(base_family = "NimbusMon") +
  xlab(expression(paste("Ratio area under the curve (Treatment / Control)"))) +
...
```{r}
strain_relabund_delta %>% group_by(Syncom, Treatment) %>% filter(!is.infinite(ratio_rel_abundance)) %>% filter(!is.nan(ratio_rel_abundance)) %>% filter(ratio_rel_abundance > 0) %>% summarize(min = min(ratio_rel_abundance), max = max(ratio_rel_abundance))
```
